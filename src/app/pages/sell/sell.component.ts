...
export class SellComponent {
  ...
  <p style="padding: 0 5%; font-size: 12px; color: #9D9480">Barrio: ${
    this.neighborhood
  }</p>
  <p style="padding: 0 5%; font-size: 12px; color: #9D9480">Dirección: ${
    this.address
  }</p>
  <p style="padding: 0 5%; font-size: 12px; color: #9D9480">Tipo: ${
    this.type
  }</p>
  <p style="padding: 0 5%; font-size: 12px; color: #9D9480">Reciclada: ${
    this.isRecycled ? 'Si' : 'No'
  }</p>
  <p style="padding: 0 5%; font-size: 12px; color: #9D9480">Metros cubiertos: ${
    this.coveredArea
  }</p>
  <p style="padding: 0 5%; font-size: 12px; color: #9D9480">Metros descubiertos: ${
    this.uncoveredArea
  }</p>`
  ...
  ngOnInit() {
    this.titleService.setTitle(
      'Vendé tu propiedad - Verónica Killian Inmobiliaria',
    )
    this.metaTagService.updateTag({
      name: 'description',
      content:
        '¿Querés vender tu propiedad? Verónica Killian te ofrece asesoramiento experto y personalizado para que puedas vender tu propiedad al mejor precio.',
    })
    this.metaTagService.updateTag({
      name: 'keywords',
      content:
    ...
    this.metaTagService.updateTag({
      property: 'og:title',
      content: 'Vendé tu propiedad - Verónica Killian Inmobiliaria',
    })
    this.metaTagService.updateTag({
  ...
  async onSubmit() {
    const notification = new Mail(
      ...
      <body style="width:100%; height: 100%; background-color: #475838; font-size: 10px; padding: 5% 0;">
        <h1 style="padding: 0 5%; font-size: 15px; color: #9D9480">Hola Verónica! - Tienes un nuevo mensaje de ${
          this.fullName
        }</h1>
        ...
        <p style="padding: 0 5%; font-size: 12px; color: #9D9480">${
          this.email
        }</p>
        <p style="padding: 0 5%; font-size: 12px; color: #9D9480">${
          this.phone
